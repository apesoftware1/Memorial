...
/**
 * @typedef {Object} PremiumListingCardProps
 * @property {Object} listing - The listing data
 * @property {string} [listing.id] - The listing ID
 * @property {string} [listing.title] - The listing title
 * @property {string} [listing.price] - The listing price
 * @property {string} [listing.image] - The listing image URL
 * @property {string} [listing.details] - The listing details
 * @property {string} [listing.features] - The listing features
 * @property {string} [listing.manufacturer] - The manufacturer name
 * @property {string} [listing.location] - The location
 * @property {string} [listing.tag] - The tag text
 * @property {string} [listing.tagColor] - The tag color
 * @property {string[]} [listing.thumbnailImages] - Array of thumbnail image URLs
 * @property {string} [href] - Optional href for the listing
 */

/**
 * @param {PremiumListingCardProps} props
 */
export function PremiumListingCard({ listing, href = "#" }) {
  const [selectedImage, setSelectedImage] = useState(0)
  
  // Convert listing to product format
  const product = {
    id: listing.id || listing.title?.replace(/\s+/g, "-").toLowerCase(),
    name: listing.title,
    price: listing.price,
    material: listing.details?.split("|")[1]?.trim() || "Granite",
    image: listing.image,
  }

  const productUrl = href || `/tombstones-for-sale/${listing.id}`
  
  // Get thumbnails from listing or create placeholders
  const thumbnails = listing.thumbnailImages || Array(6).fill("/placeholder.svg?height=80&width=80")

  return (
    <div className="flex flex-col md:flex-row gap-6 border border-gray-200 rounded-md p-4 max-w-4xl mx-auto bg-white mb-6">
