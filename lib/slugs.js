...
export function normalizeManufacturerSlugSafe(value) {
  const raw = toSlugSegment(value);
  if (!raw) return "";
  return raw;
}
...
export function buildListingCompanySlug(listing) {
  ...
  const explicitSlug = normalizeManufacturerSlugSafe(company.slug);
  if (explicitSlug) return explicitSlug;
  const nameSlug = normalizeManufacturerSlugSafe(company.name);
  if (nameSlug) return nameSlug;
  const docId = typeof company.documentId === "string" ? company.documentId.trim() : "";
  if (docId && /^[a-z0-9]{8,}$/i.test(docId)) return docId.toLowerCase();
  return "";
}

export function buildScopedListingSlug(listing) {
  const companySlug = buildListingCompanySlug(listing);
  const productSlug = buildListingCanonicalSlug(listing);
  if (!productSlug) return "";
  if (!companySlug) return productSlug;
  return `${companySlug}/${productSlug}`;
}

export function buildListingCanonicalHref(listing) {
  const scoped = buildScopedListingSlug(listing);
  if (!scoped) return null;
  return `/tombstones/${scoped}`;
}

export function buildScopedListingAbsoluteHref(listing, siteUrl) {
  const href = buildListingCanonicalHref(listing);
  if (!href) return null;
  const base = typeof siteUrl === "string" && siteUrl.trim() ? siteUrl.trim().replace(/\/+$/, "") : "";
  return base ? `${base}${href}` : href;
}

export function buildLegacyFlatListingCanonicalHref(listing) {
