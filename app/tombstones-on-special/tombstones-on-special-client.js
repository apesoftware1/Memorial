...
export default function TombstonesOnSpecialClient({ initialListings = [] }) {
  // Initialize router
  ...
  const fallbackListings = Array.isArray(initialListings) ? initialListings : [];
  ...
  const allListings = Array.isArray(data?.listings) ? data.listings : fallbackListings;
...
// Debug logging for special filtering
  ...
  if (loading && fallbackListings.length === 0) {
    return (
  ...
  // Handle error state
  if (error && fallbackListings.length === 0) {
    return (
      <div className="min-h-screen bg-gray-50">
        <Header 
          mobileMenuOpen={uiState.mobileMenuOpen}
